...
<body>
    <div class='container-fluid'>
        ...
        <div id='content-row' class='row'><div class='col-sm-12'>
            <h1>{$record[0]} {$record[1]}</h1>
            {if $too_many_images}<div class='alert alert-warning'>
                <span class='glyphicon glyphicon-picture'></span>
                <span class='glyphicon glyphicon-warning-sign'></span>
                This manuscript has
                too many images to view on one page.
                You can view 50 images at a time in the
                <a href='../gallery/?id={$id}'>
            ...
            <div id='manuscript-details-list'
                 class='collapse{if $too_many_images} in{/if}'
            >
                <dl>
                    ...
                    {if $record[5]}
                    <dt>Collation</dt>
                    <dd class='oneline'>{$record[5]}</dd>
                    {/if}
                    <dt>Form</dt>
                    <dd class='oneline'>{$record[3]}</dd>
                    <dt>Binding</dt>
                    <dd class='oneline'>{$record[4]}</dd>
                    <dt>Provenance</dt>
                    <dd><ul>{foreach $provenance as $per}<li>
                        {$per}
                    </li>{/foreach}</ul></dd>
                    {if $note}
                    <dt>Notes</dt>
                    <dd>{foreach $note as $n}<p>{$n}</p>{/foreach}</dd>
                    {/if}
                    <dt>Select bibliography</dt>
                    <dd><ul>{foreach $bib as $ref}<li>
                        {$ref}
                    </li>{/foreach}</ul></dd>
                </dl>
            ...
            <article id='part{$details[10]}'>
                ...
                {if $details[1]}<h4>by {$details[1]}</h4>{/if}
                <div class='btn-group'>
                <button type='button' class='btn btn-primary'
                        data-toggle='collapse'
                        data-target='#part{$details[10]}-details-list'
                >
                    <span class='glyphicon glyphicon-th-list'></span>
                    Show/hide details
                </button>
                ...
                <button type='button' class='btn btn-primary'
                        data-toggle='collapse'
                        data-target='#part{$details[10]}-images'
                >
                    <span class='glyphicon glyphicon-picture'></span>
                    Show/hide images
                </button>
                ...
                <div id='part{$details[10]}-details-list'
                     class='collapse'
                >
                    <dl>
                        {if $details[14]}
                        <dt>Origin</dt>
                        <dd class='oneline'>{$details[14]}</dd>
                        {/if}
                        ...
                        <dd class='oneline'>
                            ...
                            {/if}
                            {$details[3]}
                            {if $details[12] and $details[13]}</a>{/if}
                        </dd>
                        {/if}
                        {if $details[5]}
                        <dt>Dimensions</dt>
                        ...
                        {if $details[9]}
                        <dt>Attribution</dt>
                        <dd class='oneline'>{$details[9]}</dd>
                        {/if}
                    </dl>
                </div>
                {if $images[$details[11]]}
                <div id='part{$details[10]}-images'
